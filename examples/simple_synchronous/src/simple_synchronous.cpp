...
int main()
{
   ...
   hg::init_node<vk::synchronous>(
      ...
      [](const hg::data_vector &data, hg::environment &) {
         ...
         std::vector<info_job> output(n_workers, ij_in);
         for(size_t i = 0; i < n_workers; ++i) {
            output[i].start = ij_in.start + (i * delta);
            output[i].end = output[i].start + delta;
            output[i].n = ij_in.n / n_workers;
         }

         return hg::marshall_vector(output);
      },
      cg,
      env      
   );
   ...
   for(size_t i = 0; i < n_workers; ++i) {
      hg::init_node<vk::synchronous>(
         ...
         [](const hg::data_vector &data, hg::environment &) {
            ...
            double t = (ij_in.end - ij_in.start) / ij_in.n;
            double integration = 0.0;
            for(unsigned int i = 0; i < ij_in.n; ++i)
               integration += sin(ij_in.start + (t * i)) * t;
